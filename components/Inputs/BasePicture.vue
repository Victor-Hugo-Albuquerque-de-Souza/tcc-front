<template>
    <div class="photo-container form-control overflow-visible">
        <div v-if="!imageUrl" class="d-flex w-100 h-100 flex-column justify-content-center align-items-center text-center">
...
export default {
    ...
    mounted(){
        ...
        if(this.imageFromStore){
            this.imageUrl = this.imageFromStore
        }
    },
    computed: {
        imageFromStore() {
            return this.$store.state.product.images[this.order]
        }
    },
    watch: {
        // imageFromStore(newValue, oldValue) {
        //     this.imageUrl = this.imageFromStore
        // }
    }
};
